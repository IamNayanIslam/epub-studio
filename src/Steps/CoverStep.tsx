...
function getTrimmedBounds(logo: HTMLImageElement) {
  try {
    ...
    if (!tmpCtx) return { minX: 0, minY: 0, trimmedWidth: logo.naturalWidth, trimmedHeight: logo.naturalHeight };
    tmpCtx.drawImage(logo, 0, 0);
    let pixels: ImageData;
    try { pixels = tmpCtx.getImageData(0, 0, logo.naturalWidth, logo.naturalHeight); }
  ...
  return (
    <div className="grid grid-cols-1 md:grid-cols-2 gap-4 md:gap-8 items-start p-1 md:p-2">
      ...
      <div className="flex flex-col items-center gap-3">
        <h3 className={`text-[10px] font-black ${t.textMuted} uppercase tracking-widest self-start md:self-center`}>
          Live Preview
        </h3>
        ...
        <div className="flex flex-col gap-3 w-full md:items-center">
            ...
            {previewUrl ? (
              <div className="relative w-full h-full group">
                ...
                <div className="absolute transition-all duration-200 pointer-events-none" style={{
                  ...(state.coverConfig.logoPosition === "top-right" ? { top: `${state.coverConfig.margin}px` } : { bottom: `${state.coverConfig.margin}px` }),
                  right: `${state.coverConfig.margin}px`, width: `${state.coverConfig.logoSize || 18}%`,
                }}>
                  <img src={state.coverConfig.logoColor === "white" ? "/boitoi_white.png" : "/boitoi_blue.png"} alt="Logo" className="w-full h-auto drop-shadow-xl" />
                </div>
                {isInjecting && (
              ...
              <div className="flex flex-col items-center gap-2 px-2">
                <div className={`w-8 h-8 md:w-12 md:h-12 rounded-xl ${isDark ? "bg-[#252836]" : "bg-blue-50"} flex items-center justify-center`}>
                  <ImageIcon size={16} className="text-blue-400 md:hidden" />
                  <ImageIcon size={22} className="text-blue-400 hidden md:block" />
                </div>
                <p className={`text-[9px] md:text-xs font-bold ${t.textMuted} text-center`}>
          ...
          <div className="w-full md:hidden space-y-3">
            <div>
              ...
              <div className="flex gap-2">
                {(["blue", "white"] as const).map((color) => (
                  <button key={color} onClick={() => updateConfig({ logoColor: color })}
            ...
            <div>
              <label className={`block text-[9px] font-black ${t.textMuted} mb-1.5 uppercase tracking-wider`}>Position</label>
              <div className="grid grid-cols-2 gap-2">
                {[{ id: "top-right", label: "Top Right" }, { id: "bottom-right", label: "Bottom Right" }].map((pos) => (
                  <button key={pos.id} onClick={() => updateConfig({ logoPosition: pos.id })}
                    ...
                    {state.coverConfig.logoPosition === pos.id && <Check size={10} />}
                    {pos.label}
                  </button>
                ))}
              </div>
            ...
            <div>
              <div className="flex justify-between mb-1">
                <label className={`text-[9px] font-black ${t.textMuted} uppercase tracking-wider`}>Logo Size</label>
                <span className="text-blue-500 font-bold text-[10px]">{state.coverConfig.logoSize || 18}%</span>
              </div>
              <input type="range" min="10" max="40" value={state.coverConfig.logoSize || 18}
            ...
            <div>
              <div className="flex justify-between mb-1">
                <label className={`text-[9px] font-black ${t.textMuted} uppercase tracking-wider`}>Margin</label>
                <span className="text-blue-500 font-bold text-[10px]">{state.coverConfig.margin || 18}px</span>
              </div>
              <input type="range" min="0" max="100" value={state.coverConfig.margin || 18}
                onChange={(e) => updateConfig({ margin: parseInt(e.target.value) })}
                className={`w-full h-1 ${t.rangeBg} rounded-lg appearance-none cursor-pointer accent-blue-600`} />
            </div>
            {previewUrl && (
              <button onClick={handleDownloadThumbnail} disabled={isGenerating || isInjecting}
                ...
                {isGenerating ? <><RefreshCw size={12} className="animate-spin" /> Generating...</> : <><Download size={12} /> Download Thumbnail</>}
              </button>
            )}
          </div>
        </div>
      ...
      <div className={`hidden md:flex flex-col gap-6 ${t.card} border ${t.cardBorder} p-6 rounded-2xl h-full`}>

        {/* Logo Style */}
        <div>
          <label className={`block text-[10px] font-black ${t.textMuted} mb-3 uppercase tracking-[0.2em]`}>Logo Style</label>
          <div className="flex gap-2">
            {(["blue", "white"] as const).map((color) => (
              <button key={color} onClick={() => updateConfig({ logoColor: color })} className={`flex-1 ${controlBtn(state.coverConfig.logoColor === color)}`}>
        ...
        {/* Logo Position */}
        <div>
          <label className={`block text-[10px] font-black ${t.textMuted} mb-3 uppercase tracking-[0.2em]`}>Logo Position</label>
          <div className="grid grid-cols-2 gap-2">
            {[
              { id: "top-right", label: "Top Right", icon: <MousePointer2 size={12} className="rotate-[-90deg]" /> },
              { id: "bottom-right", label: "Bottom Right", icon: <MousePointer2 size={12} /> },
            ].map((pos) => (
              <button key={pos.id} onClick={() => updateConfig({ logoPosition: pos.id })} className={posBtn(state.coverConfig.logoPosition === pos.id)}>
                {state.coverConfig.logoPosition === pos.id ? <Check size={13} /> : pos.icon}
                {pos.label}
              </button>
            ))}
          </div>
        ...
        <div className="space-y-5">
          <div>
            <div className="flex justify-between mb-2">
              <label className={`flex items-center gap-1.5 text-[10px] font-black ${t.textMuted} uppercase tracking-[0.2em]`}><Maximize2 size={10} /> Logo Size</label>
              <span className={`text-blue-500 font-bold text-xs px-2 py-0.5 rounded-lg ${isDark ? "bg-blue-900/30" : "bg-blue-50"}`}>{state.coverConfig.logoSize || 18}%</span>
            </div>
            <input type="range" min="10" max="40" value={state.coverConfig.logoSize || 18}
              onChange={(e) => updateConfig({ logoSize: parseInt(e.target.value) })}
          ...
          <div>
            <div className="flex justify-between mb-2">
              <label className={`flex items-center gap-1.5 text-[10px] font-black ${t.textMuted} uppercase tracking-[0.2em]`}><Move size={10} /> Margin Offset</label>
              <span className={`text-blue-500 font-bold text-xs px-2 py-0.5 rounded-lg ${isDark ? "bg-blue-900/30" : "bg-blue-50"}`}>{state.coverConfig.margin || 18}px</span>
            </div>
            <input type="range" min="0" max="100" value={state.coverConfig.margin || 18}
              onChange={(e) => updateConfig({ margin: parseInt(e.target.value) })}
              className={`w-full h-1.5 ${t.rangeBg} rounded-lg appearance-none cursor-pointer accent-blue-600`} />
        ...
        <div className="mt-auto pt-2">
          ...
          >
            {isGenerating ? <><RefreshCw size={15} className="animate-spin" /> Generating...</> : <><Download size={15} /> Download Thumbnail</>}
          </button>
        </div>
      </div>
    </div>
