...
// Extend CoverConfig locally to include addBackground
type ExtendedCoverConfig = {
  logoColor: string;
  logoSize: number;
  margin: number;
  logoPosition: string;
...
const CoverStep = () => {
  ...
  const coverConfig = state.coverConfig as ExtendedCoverConfig;
  ...
  return (
    <div className="grid grid-cols-1 md:grid-cols-2 gap-4 md:gap-8 items-start p-1 md:p-2">
            ...
            {previewUrl ? (
              <div className="relative w-full h-full group">
                ...
                <div
                  ...
                  style={{
                    ...(coverConfig.logoPosition === "top-right"
                      ? { top: `${coverConfig.margin}px` }
                      : { bottom: `${coverConfig.margin}px` }),
                    right: `${coverConfig.margin}px`,
                    width: `${coverConfig.logoSize || 18}%`,
                  }}
                >
                  {coverConfig.addBackground ? (
                    ...
                    >
                      <img
                        src={coverConfig.logoColor === "white" ? "/boitoi_white.png" : "/boitoi_blue.png"}
                        alt="Logo"
      ...
      <div className="hidden md:flex flex-col gap-6 h-full">
        ...
        {/* Logo Style */}
        <div>
          <p className={`text-[10px] font-black ${t.textMuted} mb-3 uppercase tracking-[0.2em]`}>Logo Style</p>
          <div className="flex gap-2">
            {(["blue", "white"] as const).map((color) => (
              <button key={color} onClick={() => updateConfig({ logoColor: color })} className={`flex-1 ${controlBtn(coverConfig.logoColor === color)}`}>
                {coverConfig.logoColor === color && <Check size={13} />}
        ...
        {/* Logo Position */}
        <div>
          <p className={`text-[10px] font-black ${t.textMuted} mb-3 uppercase tracking-[0.2em]`}>Logo Position</p>
          <div className="flex gap-2">
            {[
              { id: "top-right", label: "Top Right", icon: <MousePointer2 size={13} className="rotate-[-90deg]" /> },
              { id: "bottom-right", label: "Bottom Right", icon: <MousePointer2 size={13} /> },
            ].map((pos) => (
              <button key={pos.id} onClick={() => updateConfig({ logoPosition: pos.id })} className={`flex-1 ${posBtn(coverConfig.logoPosition === pos.id)}`}>
                {coverConfig.logoPosition === pos.id ? <Check size={13} /> : pos.icon}
                {pos.label}
              </button>
            ))}
          </div>
        ...
        <div>
          <label className="flex items-center gap-3 cursor-pointer group">
            ...
            >
              {coverConfig.addBackground && <Check size={12} className="text-white" />}
            </div>
            <div>
              <p className={`text-sm font-bold ${t.textPrimary}`}>Add Background</p>
              <p className={`text-[10px] ${t.textMuted}`}>
                {coverConfig.logoColor === "white"
        ...
        <div>
          <div className="flex justify-between mb-2">
            <p className={`flex items-center gap-1.5 text-[10px] font-black ${t.textMuted} uppercase tracking-[0.2em]`}>
