...
async function buildCoverBlob(coverUrl: string): Promise<Blob> {
  ...
  try {
    ...
    return new Promise((res, rej) =>
      canvas.toBlob((b) => (b ? res(b) : rej("failed")), "image/jpeg", 0.85),
    );
  }
}
...
export const CoverToolModal = ({
  ...
  const controlBtn = (active: boolean) =>
    `flex-1 py-2.5 rounded-xl border-2 flex items-center justify-center gap-1.5 transition-all font-bold text-xs ${
      active
        ...
        : isDark
          ? "border-[#2A2D3E] bg-[#252836] text-gray-400"
  ...
  return (
    <div className="fixed inset-0 z-[110] flex flex-col">
      {/* Header */}
      <div
        className={`${t.header} border-b ${t.cardBorder} px-4 sm:px-6 py-3 sm:py-4 flex items-center justify-between shrink-0`}
      >
        <div>
          <h2 className={`text-sm sm:text-base font-black ${t.textPrimary}`}>
            Cover Generator
          </h2>
          <p
      ...
      <div
        className={`flex-1 overflow-hidden ${t.bg} flex flex-col sm:flex-row`}
      >
        {/* ── Preview panel ── */}
        <div className="sm:flex-1 flex flex-col items-center justify-start sm:justify-center p-4 sm:p-6 overflow-y-auto sm:overflow-auto">
            ...
            {previewUrl ? (
              <div className="relative w-full h-full group">
                ...
                <div className="absolute inset-0 bg-black/40 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center">
                  <p className="text-white text-sm font-bold bg-black/50 px-4 py-2 rounded-xl">
                    Change Image
                  </p>
                </div>
              </div>
              ...
              <div className="w-full h-full flex flex-col items-center justify-center gap-3">
                <div
                  className={`w-12 h-12 sm:w-16 sm:h-16 rounded-2xl ${isDark ? "bg-[#252836]" : "bg-blue-50"} flex items-center justify-center`}
                >
                ...
                <div className="text-center px-4">
                  <p
                    className={`text-xs sm:text-sm font-bold ${t.textPrimary}`}
                  >
          ...
          <p
            className={`mt-2 text-[10px] font-bold ${t.textMuted} uppercase tracking-widest`}
          >
            395 × 632 px
          </p>
          ...
          <div className="sm:hidden w-full max-w-[395px] mt-4 grid grid-cols-2 gap-2.5">
            <button
              ...
              className={`flex items-center justify-center gap-1.5 py-3 rounded-xl font-bold text-xs border-2 transition-all active:scale-95 disabled:opacity-40 ${
                isDark
                  ? "border-[#2A2D3E] bg-[#252836] text-gray-300 hover:border-blue-500 hover:text-blue-400"
          ...
          <div
            className={`sm:hidden w-full max-w-[395px] mt-4 border ${t.cardBorder} rounded-2xl p-4 space-y-4 ${t.card}`}
          >
            {/* Logo Style */}
            <div>
              <label
                className={`block text-[10px] font-black ${t.textMuted} mb-2 uppercase tracking-[0.2em]`}
              >
                Logo Style
              </label>
              <div className="flex gap-2">
                {(["blue", "white"] as const).map((color) => (
                  <button
                    ...
                    className={controlBtn(config.logoColor === color)}
                  >
                ...
                {[
                  {
                    ...
                    label: "Top Right",
                    icon: (
                      <MousePointer2 size={11} className="rotate-[-90deg]" />
                    ),
                  },
                  {
                    id: "bottom-right",
                  ...
                  <button
                    ...
                    className={posBtn(config.logoPosition === pos.id)}
                  >
            ...
            <label className="flex items-center gap-3 cursor-pointer">
              <div
                onClick={() =>
                  updateConfig({ addBackground: !config.addBackground })
                }
              ...
              {config.addBackground && (
                <div
                  className="ml-auto w-5 h-5 rounded-sm border border-gray-300 shrink-0"
                  style={{ backgroundColor: bgPreviewColor }}
                />
              )}
            </label>
        ...
        <div
          className={`hidden sm:flex w-72 shrink-0 flex-col border-l ${t.cardBorder} ${t.card} overflow-y-auto`}
        >
          <div className="p-5 space-y-5 flex-1">
            {/* Logo Style */}
            <div>
              <label
                className={`block text-[10px] font-black ${t.textMuted} mb-2.5 uppercase tracking-[0.2em]`}
              >
                Logo Style
              </label>
              <div className="flex gap-2">
            ...
            <div>
              <label className="flex items-center gap-3 cursor-pointer">
                ...
                {config.addBackground && (
                  <div
                    className="ml-auto w-5 h-5 rounded-sm border border-gray-300"
                    style={{ backgroundColor: bgPreviewColor }}
                  />
                )}
              </label>
            ...
            {/* Info */}
            <div
              className={`p-3 rounded-xl ${isDark ? "bg-blue-900/20 border border-blue-800/30" : "bg-blue-50 border border-blue-100"}`}
            >
              <p
                className={`text-[10px] font-bold ${isDark ? "text-blue-400" : "text-blue-600"} leading-relaxed`}
              >
                <strong>Cover</strong> — লোগো ছাড়া
                <br />
                <strong>Thumbnail</strong> — লোগো সহ
              </p>
            </div>
          </div>
