...
export const CoverToolModal = ({ isOpen, onClose, theme: t, isDark }: Props) => {
  ...
  return (
    <div className="fixed inset-0 z-[110] flex flex-col">
      {/* Header */}
      <div className={`${t.header} border-b ${t.cardBorder} px-4 sm:px-6 py-3 sm:py-4 flex items-center justify-between shrink-0`}>
        <div>
          <h2 className={`text-sm sm:text-base font-black ${t.textPrimary}`}>Cover Generator</h2>
      ...
      {/* Body — mobile: scroll vertically, desktop: side by side */}
      <div className={`flex-1 overflow-hidden ${t.bg} flex flex-col sm:flex-row`}>

        {/* ── Preview panel ── */}
        <div className="sm:flex-1 flex flex-col items-center justify-start sm:justify-center p-4 sm:p-6 overflow-y-auto sm:overflow-auto">
          <div
            {...getRootProps()}
            className={`relative cursor-pointer rounded-2xl overflow-hidden border-2 border-dashed transition-all shadow-xl sm:shadow-2xl ${
              previewUrl ? "border-blue-500 ring-2 sm:ring-4 ring-blue-500/10"
                : isDragActive ? t.dropzoneActive : t.dropzone
            }`}
            style={{
              // Mobile: fit within screen, Desktop: real 395×632
              width: "min(calc(100vw - 32px), 395px)",
              aspectRatio: "395 / 632",
              flexShrink: 0,
            }}
          >
            ...
            {previewUrl ? (
                ...
                >
                  {config.addBackground ? (
                    <div className="p-[5px] rounded-md inline-block" style={{ backgroundColor: bgPreviewColor }}>
                      <img src={config.logoColor === "white" ? "/boitoi_white.png" : "/boitoi_blue.png"} alt="Logo" className="w-full h-auto block" />
                    </div>
                  ) : (
                    <img src={config.logoColor === "white" ? "/boitoi_white.png" : "/boitoi_blue.png"} alt="Logo" className="w-full h-auto drop-shadow-xl" />
                  )}
                </div>
              ...
              <div className="w-full h-full flex flex-col items-center justify-center gap-3">
                <div className={`w-12 h-12 sm:w-16 sm:h-16 rounded-2xl ${isDark ? "bg-[#252836]" : "bg-blue-50"} flex items-center justify-center`}>
                  <ImageIcon size={22} className="text-blue-400 sm:hidden" />
                  <ImageIcon size={28} className="text-blue-400 hidden sm:block" />
                </div>
                <div className="text-center px-4">
                  <p className={`text-xs sm:text-sm font-bold ${t.textPrimary}`}>{isDragActive ? "Drop করুন!" : "Click or Drag Image"}</p>
                  <p className={`text-[10px] ${t.textMuted} mt-1`}>JPG · PNG · WEBP</p>
                </div>
              </div>
            )}
          </div>
          <p className={`mt-2 text-[10px] font-bold ${t.textMuted} uppercase tracking-widest`}>
            395 × 632 px
          </p>

          {/* Mobile download buttons — under preview */}
          <div className="sm:hidden w-full max-w-[395px] mt-4 grid grid-cols-2 gap-2.5">
            <button
              onClick={() => handleDownload("cover")}
              disabled={!previewUrl || isGenerating}
              className={`flex items-center justify-center gap-1.5 py-3 rounded-xl font-bold text-xs border-2 transition-all active:scale-95 disabled:opacity-40 ${
            ...
            <button
              ...
              disabled={!previewUrl || isGenerating}
              className="flex items-center justify-center gap-1.5 py-3 rounded-xl font-bold text-xs bg-blue-600 text-white hover:bg-blue-700 shadow-lg shadow-blue-500/20 transition-all active:scale-95 disabled:opacity-40"
            >
              {isGenerating ? <Loader2 size={13} className="animate-spin" /> : <Download size={13} />}
              Thumbnail
            </button>
          ...
          {/* Mobile controls — under download buttons */}
          <div className={`sm:hidden w-full max-w-[395px] mt-4 border ${t.cardBorder} rounded-2xl p-4 space-y-4 ${t.card}`}>
            {/* Logo Style */}
            <div>
              <label className={`block text-[10px] font-black ${t.textMuted} mb-2 uppercase tracking-[0.2em]`}>Logo Style</label>
            ...
            {/* Logo Position */}
            <div>
              <label className={`block text-[10px] font-black ${t.textMuted} mb-2 uppercase tracking-[0.2em]`}>Logo Position</label>
              <div className="flex gap-2">
                {[
            ...
            <label className="flex items-center gap-3 cursor-pointer">
              ...
              {config.addBackground && (
                <div className="ml-auto w-5 h-5 rounded-sm border border-gray-300 shrink-0" style={{ backgroundColor: bgPreviewColor }} />
              )}
            </label>
            ...
            <div>
              <div className="flex justify-between mb-2">
                ...
                <span className={`text-blue-500 font-bold text-xs px-2 py-0.5 rounded-lg ${isDark ? "bg-blue-900/30" : "bg-blue-50"}`}>{config.logoSize}%</span>
              </div>
            ...
            <div>
              <div className="flex justify-between mb-2">
                ...
                <span className={`text-blue-500 font-bold text-xs px-2 py-0.5 rounded-lg ${isDark ? "bg-blue-900/30" : "bg-blue-50"}`}>{config.margin}px</span>
              </div>
        ...
        <div className={`hidden sm:flex w-72 shrink-0 flex-col border-l ${t.cardBorder} ${t.card} overflow-y-auto`}>
