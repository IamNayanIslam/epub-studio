...
async function pngToJpeg(pngData: ArrayBuffer): Promise<Uint8Array> {
  ...
  return new Promise((resolve, reject) => {
    ...
    img.onload = async () => {
      ...
      try {
        const jpegBuffer = await jpeg.encode(imageData, {
...
async function compressEpub(
  ...
  for (const filePath of Object.keys(content.files)) {
    ...
    if (lp.endsWith(".png") && !isCover) {
      ...
      (lp.endsWith(".jpg") || lp.endsWith(".jpeg") || lp.endsWith(".webp")) &&
      !isCover
    ) {
  ...
  for (const [filePath, zipFile] of Object.entries(content.files)) {
    ...
    if (toCompress.has(filePath)) {
      try {
        ...
        const outPath =
          renameMap[filePath] ?? filePath.replace(/\.(webp)$/i, ".jpg");
...
export const CompressEpubModal = ({
  isOpen,
  onClose,
  theme: t,
  isDark,
  ...
  return (
    <div className="fixed inset-0 z-[110] flex items-end sm:items-center justify-center p-0 sm:p-4">
      ...
      <div
        className={`${t.card} border ${t.cardBorder} w-full max-w-lg rounded-t-3xl sm:rounded-[28px] shadow-2xl z-10 overflow-hidden max-h-[92vh] overflow-y-auto`}
      >
        {/* Header */}
        <div className="relative p-6 sm:p-8 pb-4 sm:pb-5">
          ...
          <div className="flex flex-col items-center text-center">
            ...
            <h2
              className={`text-xl sm:text-2xl font-black tracking-tight ${t.textPrimary}`}
            >
              Compress EPUB
            </h2>
            <p
              className={`text-xs font-bold mt-1 uppercase tracking-widest ${t.textMuted}`}
            >
        ...
        <div className="px-5 sm:px-8 pb-6 sm:pb-8 space-y-4">
          ...
          >
            ...
            {file ? (
              <div className="flex items-center gap-3">
                <div
                  className={`w-10 h-10 rounded-xl flex items-center justify-center ${isDark ? "bg-orange-900/30" : "bg-orange-100"} text-orange-500 shrink-0`}
                >
                  <FileArchive size={20} />
                </div>
                <div className="text-left min-w-0">
                  <p className={`text-sm font-black truncate ${t.textPrimary}`}>
                    {file.name}
                  </p>
                ...
                <p
                  className={`ml-auto text-xs font-bold ${t.textMuted} shrink-0`}
                >
                  Change
                </p>
              </div>
          ...
          {!result && (
            <div
              className={`p-3 rounded-xl flex gap-2 ${isDark ? "bg-blue-900/20 border border-blue-800/30" : "bg-blue-50 border border-blue-100"}`}
            >
              ...
              >
                সব <strong>.png</strong> এবং <strong>100KB+ বড়</strong>
                JPG/WEBP images compress করবে (cover ছাড়া)। PNG হলে filename ও
                <strong>.jpg</strong> তে rename হবে এবং সব reference update হবে।
              </p>
            </div>
          )}
          ...
          {result && (
            <div
              className={`p-4 rounded-2xl border ${isDark ? "border-green-800/30 bg-green-900/10" : "border-green-200 bg-green-50"}`}
            >
              <div className="flex items-center gap-2 mb-3">
                <CheckCircle2 size={18} className="text-green-500" />
                <p
                  className={`font-black text-sm ${isDark ? "text-green-400" : "text-green-700"}`}
                >
                  Processing Complete!
                </p>
              </div>
              <div className="grid grid-cols-2 gap-3">
                <div
                  className={`p-3 rounded-xl ${isDark ? "bg-black/20" : "bg-white"} text-center`}
                >
                  <p
                    className={`text-[10px] font-black uppercase tracking-wider ${t.textMuted}`}
                  >
                    Converted
                  </p>
                  <p
                ...
                >
                  ...
                  <p
                    className={`text-2xl font-black ${isDark ? "text-orange-400" : "text-orange-600"}`}
                  >
                    {savedPercent}%
                  </p>
                  <p className={`text-[10px] ${t.textMuted}`}>
                    {formatSize(saved)} saved
                  </p>
                </div>
              </div>
              <div
                className={`mt-3 flex items-center justify-center gap-2 text-xs font-bold ${t.textMuted}`}
              >
